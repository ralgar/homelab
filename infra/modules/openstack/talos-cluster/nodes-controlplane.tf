resource "openstack_compute_instance_v2" "controlplane" {
  ...
  // Attach additional block devices to control plane ONLY if there are no workers
  dynamic "block_device" {
    for_each = var.worker_count == 0 ? toset(range(var.extra_volume_count)) : toset([])
    content {
      source_type           = "blank"
      destination_type      = "volume"
...
// Optional floating IP for control plane
resource "openstack_networking_floatingip_v2" "controlplane" {
